...
/**
 * Class AnnouncementType Entity Form.
 */
class AnnouncementTypeForm extends EntityForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        \Drupal::messenger()->addStatus($this->t('Created the %label Announcement type.', [
          '%label' => $announcement_type->label(),
        ]));
        break;

      default:
        \Drupal::messenger()->addStatus($this->t('Saved the %label Announcement type.', [
          '%label' => $announcement_type->label(),
        ]));
    }
